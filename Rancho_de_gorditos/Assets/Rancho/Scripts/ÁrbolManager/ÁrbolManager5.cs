...
public class ÁrbolManager5 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        ÁrbolUI = GameObject.Find("ÁrbolUI5");
    ...
    private void Crecer()
    {
        ...
        ÁrbolUI5 a = ÁrbolUI.GetComponent<ÁrbolUI5>();
        if (c.dia != dia)
        {
            if (plantado)
            {
                ...
                if (fase == 1)
                {
                    ...
                    {
                        Destroy(semi);
                        semi = Instantiate(Naranjo, hueco.transform.position, hueco.transform.rotation);
